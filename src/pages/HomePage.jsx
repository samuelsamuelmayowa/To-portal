...
const socialLinks = [
  ...
  { label: "Email", href: "mailto:hello@toanalytics.com" },
];

const HomePage = () => (
  <main className="relative min-h-screen overflow-hidden bg-[#050816] text-white">
    <Helmet>
      <title>We’ll Be Back Soon | T.O Analytics</title>
      <meta name="description" content="T.O Analytics is currently undergoing scheduled maintenance. We’ll be back online soon with an improved experience." />
    ...
    <div className="pointer-events-none absolute inset-0">
      ...
      <div className="absolute inset-0 opacity-[0.06]" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.35) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.35) 1px, transparent 1px)", backgroundSize: "52px 52px", maskImage: "linear-gradient(to bottom, black 0%, transparent 85%)" }} />
    </div>

    <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 py-8 sm:px-10 lg:px-16">
      <motion.header initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex items-center justify-between">
        <a href="/" className="group flex items-center gap-3" aria-label="T.O Analytics home">
          <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-400 text-sm font-black shadow-lg shadow-blue-500/20 transition-transform group-hover:scale-105">T.O</span>
          <span>
            <span className="block text-base font-black tracking-tight">T.O Analytics</span>
            <span className="block text-[10px] font-bold uppercase tracking-[0.24em] text-slate-400">Learn · Build · Grow</span>
          </span>
        </a>
        <div className="hidden items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 sm:flex">
          ...
          <span className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-300">Systems upgrading</span>
        </div>
      </motion.header>

      <section className="grid flex-1 items-center gap-14 py-16 lg:grid-cols-[1.15fr_.85fr] lg:py-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, delay: 0.1 }}>
          <div className="mb-7 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 backdrop-blur-xl">
            <span className="text-lg">🛠️</span><span className="text-xs font-extrabold uppercase tracking-[0.22em] text-cyan-300">Scheduled maintenance</span>
          </div>
          <h1 className="max-w-4xl text-5xl font-black leading-[0.98] tracking-[-0.045em] sm:text-6xl lg:text-7xl xl:text-8xl">
            Something better is <span className="block bg-gradient-to-r from-blue-400 via-cyan-300 to-sky-400 bg-clip-text text-transparent">on the way.</span>
          </h1>
          <p className="mt-7 max-w-2xl text-base font-medium leading-8 text-slate-300 sm:text-lg">We’re improving the T.O Analytics experience to make it faster, smarter and even more valuable for our learners and partners. We’ll be back online very soon.</p>
          <div className="mt-9 flex flex-col gap-4 sm:flex-row sm:items-center">
            <a href="mailto:hello@toanalytics.com" className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 px-7 py-4 text-sm font-extrabold shadow-xl shadow-blue-900/30 transition duration-300 hover:-translate-y-1 hover:shadow-cyan-500/20">Contact our team <span className="ml-3 text-lg" aria-hidden="true">→</span></a>
            <p className="text-sm font-semibold text-slate-400">Need urgent help? We’re still available.</p>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, scale: 0.9, rotate: 2 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ duration: 0.85, delay: 0.2 }} className="relative mx-auto w-full max-w-lg">
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.07] p-5 shadow-2xl shadow-black/40 backdrop-blur-2xl sm:p-7">
            <div className="flex items-center gap-2 border-b border-white/10 pb-5"><span className="h-3 w-3 rounded-full bg-red-400" /><span className="h-3 w-3 rounded-full bg-amber-300" /><span className="h-3 w-3 rounded-full bg-emerald-400" /><div className="ml-3 h-7 flex-1 rounded-lg bg-white/[0.06]" /></div>
            <div className="py-9 text-center">
              <div className="relative mx-auto flex h-32 w-32 items-center justify-center">
                <motion.div animate={{ rotate: 360 }} transition={{ duration: 12, repeat: Infinity, ease: "linear" }} className="absolute inset-0 rounded-full border border-dashed border-cyan-300/50" />
                <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }} className="flex h-24 w-24 items-center justify-center rounded-[2rem] bg-gradient-to-br from-blue-600 to-cyan-400 text-4xl shadow-2xl shadow-blue-500/30">⚙️</motion.div>
              </div>
              <p className="mt-7 text-xs font-extrabold uppercase tracking-[0.25em] text-cyan-300">Upgrade in progress</p>
              <h2 className="mt-3 text-2xl font-black">Building a better experience</h2>
              <div className="mt-7 h-2 overflow-hidden rounded-full bg-white/10"><motion.div initial={{ width: "18%" }} animate={{ width: ["18%", "82%", "45%", "92%"] }} transition={{ duration: 8, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }} className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-300" /></div>
              <div className="mt-3 flex justify-between text-[11px] font-bold uppercase tracking-widest text-slate-500"><span>Optimizing</span><span>Almost there</span></div>
            </div>
          </div>
        </motion.div>
      </section>

      <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.6 }} className="flex flex-col gap-5 border-t border-white/10 py-6 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between">
        <p>© {new Date().getFullYear()} T.O Analytics. All rights reserved.</p>
        <nav className="flex flex-wrap gap-x-6 gap-y-2" aria-label="Social links">{socialLinks.map((link) => <a key={link.label} href={link.href} target={link.href.startsWith("http") ? "_blank" : undefined} rel={link.href.startsWith("http") ? "noreferrer" : undefined} className="font-bold text-slate-400 transition hover:text-cyan-300">{link.label}</a>)}</nav>
      </motion.footer>
    </div>
  </main>
);
