...
const LABS = [
  {
    ...
    hint: "Filter using the keyword ERROR",
    starter: "index=_internal ERROR",
    expected: (q, res) =>
      q.includes("index=_internal") &&
      q.toUpperCase().includes("ERROR") &&
      res?.count > 0,
  },
  {
    id: "lab-2",
    title: "Count Events by Sourcetype",
    goal: "Return a table showing event counts per sourcetype.",
    hint: "Use: stats count by sourcetype",
    starter: "index=_internal | stats count by sourcetype",
    expected: (q, res) =>
      q.includes("stats count by sourcetype") && res?.mode === "table",
  },
  {
    id: "lab-3",
    title: "Detect Failed Logins (SOC)",
    goal: "Find failed login attempts in auth logs.",
    hint: "Filter using status=FAILED",
    starter: "index=auth_logs status=FAILED",
    expected: (q, res) =>
      q.includes("index=auth_logs") &&
      q.includes("status=FAILED") &&
      res?.count > 0,
  },
  {
    id: "lab-4",
    title: "Top Attacking IPs",
    goal: "Show the top source IPs attempting logins.",
    hint: "Use: top src_ip limit=10",
    starter: "index=auth_logs status=FAILED | top src_ip limit=10",
    expected: (q, res) =>
      q.includes("top src_ip") && res?.mode === "table",
  },
  {
    id: "lab-5",
    title: "Errors Over Time",
    goal: "Visualize errors using timechart.",
    hint: "Use: timechart span=15m count",
    starter: "index=_internal ERROR | timechart span=15m count",
    expected: (q, res) =>
      q.includes("timechart") && res?.mode === "timechart",
  },
];

/* ================= MINI BAR CHART ================= */
function MiniBarChart({ rows, xKey, yKey }) {
  const max = Math.max(...rows.map((r) => Number(r[yKey] || 0)), 1);

  return (
    <div className="flex items-end gap-2 h-36">
      {rows.map((r, i) => {
        const h = Math.round((r[yKey] / max) * 140);
        return (
          <div key={i} className="flex-1 text-center">
            <div
              className="mx-auto rounded-md bg-cyan-500/80"
              style={{ height: h }}
            />
...
export default function SplunkPracticeLab() {
  ...
  const runSearch = () => {
    ...
    setResult(res);
    setActiveTab(
      res.mode === "timechart" ? "chart" : res.mode === "table" ? "table" : "events"
    );
    ...
    <div className="max-w-7xl mx-auto py-12 text-white">
      ...
      <div className="mb-10">
        <h1 className="text-4xl font-bold flex items-center gap-3">
          🧪 Splunk Practice Lab
          <span className="px-3 py-1 rounded-full text-xs bg-cyan-500/10 border border-cyan-400/30 text-cyan-300">
            PRO
          </span>
          <Sparkles className="text-cyan-400" size={20} />
        </h1>
        <p className="text-gray-400 mt-2 max-w-2xl">
          A simulated live Splunk environment where students practice SPL,
          SOC detection, dashboards, and alerts — safely.
        </p>
      </div>

      <div className="grid lg:grid-cols-[320px_1fr] gap-8">
        {/* ================= LEFT: LABS ================= */}
        <aside className="bg-[#0f172a]/80 backdrop-blur border border-white/10 rounded-2xl p-5">
          <h3 className="text-lg font-semibold mb-3">🎯 Guided Missions</h3>
          ...
          {labStatus !== null && (
            ...
            >
              {labStatus ? (
                ...
                <div className="flex items-center gap-2">
                  <AlertTriangle size={16} /> Try again — {activeLab.hint}
                </div>
              )}
            </div>
          )}
        ...
        <main>
          {/* SEARCH BAR */}
          <div className="relative bg-[#020617]/80 backdrop-blur border border-white/10 p-4 rounded-2xl mb-5">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={runSearch}
                className="bg-cyan-600 px-5 py-2 rounded-lg text-sm hover:bg-cyan-500"
              >
                Search
              </button>
            </div>
            ...
            {showSuggestions && filteredSuggestions.length > 0 && (
              <div className="absolute left-4 right-4 mt-3 bg-[#020617] border border-white/10 rounded-xl overflow-hidden">
                {filteredSuggestions.map((s, i) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-xs font-mono hover:bg-cyan-500/10"
                  >
                    {s}
                  </button>
                ))}
          ...
          {/* ERROR */}
          {error && (
            <div className="mb-4 p-3 rounded-xl border border-red-600 bg-red-600/10 text-sm">
              ❌ {error}
            </div>
          )}

          {/* RESULTS */}
          <div className="bg-[#0f172a]/90 backdrop-blur border border-white/10 rounded-2xl p-5">
            {!result ? (
              <p className="text-gray-400 text-sm">
                Run a search or complete a guided mission to see results.
              </p>
            ) : activeTab === "chart" ? (
              <MiniBarChart
                ...
                <thead>
                  <tr className="text-gray-400">
                    {Object.keys(rows[0] || {}).slice(0, 8).map((k) => (
                      <th key={k} className="pb-2 text-left">{k}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
