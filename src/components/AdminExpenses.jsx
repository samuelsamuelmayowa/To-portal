...
const initialExpenses = [
  ...
  },
  {
    ...
    renewalDate: "",
    notes:
      "Domain renewal costs ₦38,000 and expires this month, August 2026. Add the exact renewal date when confirmed.",
  },
  {
    id: "frontend-hosting",
...
function loadExpenses() {
  try {
    ...
    */
    ...
    const domainExpenseIndex =
      migratedExpenses.findIndex(
        (expense) =>
          expense.id === "domain",
      );

    if (domainExpenseIndex === -1) {
      const domainExpense =
        initialExpenses.find(
          (expense) =>
            expense.id === "domain",
        );
    ...
    return migratedExpenses.map((expense) => {
      if (expense.id === "domain") {
        const storedDomainAmount = Number(
          expense.amount,
        );

        const domainStillHasOldPrice =
          storedDomainAmount === 50000 ||
          expense.amount === "" ||
          expense.amount === null ||
          expense.amount === undefined;

        if (!domainStillHasOldPrice) {
          return expense;
        }
